...
const navGroups = [
  {
    ...
    items: [
      ...
      { label: 'Rounds', path: '/rounds', icon: Database },
  ...
  },
  {
    label: 'Security',
    items: [
      { label: 'Threat analysis', path: '/threats', icon: ShieldAlert },
      { label: 'Impact analysis', path: '/impact', icon: BarChart3 },
      { label: 'Recovery', path: '/recovery', icon: RotateCcw },
      { label: 'Validation', path: '/validation', icon: FlaskConical },
    ],
  },
  {
    label: 'System',
    items: [{ label: 'Audit log', path: '/audit', icon: ClipboardList }],
  },
] as const

function normalizeRoute(path: string): AppRoute {
  return validRoutes.includes(path as AppRoute) ? (path as AppRoute) : '/dashboard'
}
...
export default function App() {
  ...
  const [theme, setTheme] = useState<Theme>(
    () => (localStorage.getItem('fedsentinel-theme') === 'light' ? 'light' : 'dark'),
  )
  ...
  const [collapsed, setCollapsed] = useState(
    () => localStorage.getItem('fedsentinel-sidebar') === 'collapsed',
  )
  ...
  const loadRuns = useCallback(async (preferRun?: string) => {
    try {
  ...
  useEffect(() => {
    localStorage.setItem('fedsentinel-sidebar', collapsed ? 'collapsed' : 'open')
  }, [collapsed])
  ...
  const page = useMemo(() => {
    if (!selectedRun && route !== '/simulation' && route !== '/validation') {
      return <NoRunPage onNavigate={navigate} />
    }
    switch (route) {
      case '/simulation':
        return <SimulationPage data={dashboard.data} onStart={startSimulation} starting={starting} />
      case '/rounds':
        return <RoundsPage data={dashboard.data} />
      case '/clients':
        return <ClientsPage data={dashboard.data} />
      case '/threats':
        return <ThreatsPage data={dashboard.data} />
      case '/impact':
        return <ImpactPage data={dashboard.data} />
      case '/recovery':
        return <RecoveryPage data={dashboard.data} />
      case '/validation':
        return <ValidationPage data={dashboard.data} />
      case '/audit':
        return <AuditPage data={dashboard.data} />
      default:
        return (
          <Dashboard
            data={dashboard.data}
            refreshing={dashboard.refreshing}
            onRefresh={() => void dashboard.refresh(false)}
            onNavigate={navigate}
          />
        )
    }
  }, [route, selectedRun, dashboard.data, dashboard.refreshing, dashboard.refresh, starting, navigate])
  ...
  return (
    <div className={`app-shell ${collapsed ? 'sidebar-collapsed' : ''}`}>
          ...
          <div className="connection-card">
            <span
              className={`connection-dot ${
                apiOnline ? 'online' : apiOnline === false ? 'offline' : ''
              }`}
            />
            <div>
              <strong>
      ...
      <div className="workspace">
              ...
              >
                ...
                {runs.map((run) => (
                  <option key={run.run_id} value={run.run_id}>
                    {run.run_id} · {run.scenario.replaceAll('_', ' ')}
                  </option>
                ))}
              </select>
              {selectedSummary && <StatusBadge value={selectedSummary.status} />}
            </div>
          </div>
          <div className="sync-meta">
            <span className={dashboard.refreshing ? 'pulse' : ''} />
            <div>
              <strong>{autoRefresh ? 'Live refresh on' : 'Live refresh paused'}</strong>
              <small>
                {dashboard.lastUpdated
                  ? `Synced ${dateTime(dashboard.lastUpdated.toISOString())}`
                  : 'Awaiting first sync'}
              </small>
            </div>
          </div>
        ...
        {(startupError || dashboard.error) && (
          <div className="error-banner" role="alert">
            <ShieldAlert />
            <div>
              <strong>Backend request failed</strong>
              <span>{startupError || dashboard.error}</span>
            </div>
            <button
              type="button"
              onClick={() => {
