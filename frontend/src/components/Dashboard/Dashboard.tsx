...
export function Dashboard({
      ...
      <div className="export-menu">
        ...
        {runId && (
          <div className="export-options">
            <button type="button" onClick={() => exportAnalyticsCsv(data, runId)}>
              Analytics CSV
            </button>
            <button type="button" onClick={() => exportRecoveryCsv(data, runId)}>
              Recovery CSV
            </button>
            <button type="button" onClick={() => exportJson(data, runId)}>
              Complete JSON
            </button>
          </div>
        )}
  ...
  return (
    <main className="page">
      <PageHeader
        eyebrow="Federated defense analytics"
        title="Security Operations Center"
        description="Live operational truth from the FedSentinel backend."
        actions={actions}
      />
      <section className="run-status-strip">
        <div>
          <span>Run state</span>
        ...
        <div>
          <span>Round</span>
          <strong>{data.run ? `${data.run.current_round} / ${data.run.round_count}` : '—'}</strong>
        </div>
        <div>
          <span>Clients</span>
          <strong>{data.run?.client_count ?? '—'}</strong>
        </div>
        <div>
          <span>Scenario</span>
          <strong>{data.run?.scenario?.replaceAll('_', ' ') || '—'}</strong>
        </div>
        <div>
          <span>Suspicious</span>
          <strong className="semantic-warning">{roundThreats.length ? suspicious : '—'}</strong>
        </div>
        <div>
          <span>Malicious</span>
          <strong className="semantic-danger">{roundThreats.length ? malicious : '—'}</strong>
        </div>
      </section>
      <section className="metric-grid metric-grid-five">
        <MetricCard
          label="Model accuracy"
          value={percent(latest?.accuracy)}
          context={latest ? `Round ${latest.round_id} evaluation` : 'No evaluation yet'}
          tone="success"
          icon={<TrendingUp />}
        />
        <MetricCard
          label="Model loss"
          value={decimal(latest?.loss)}
          context="Persisted model evaluation"
          icon={<Activity />}
        />
        <MetricCard
          label="Attack success"
          value={percent(latest?.attack_success_rate)}
          context={latest?.attack_success_rate == null ? 'Not available' : 'Accepted attacked updates'}
          tone="warning"
          icon={<Crosshair />}
        />
        <MetricCard
          label="Detected threats"
          value={roundThreats.length ? String(suspicious + malicious) : '—'}
          context="Suspicious and malicious"
          tone={malicious ? 'danger' : 'neutral'}
          icon={<AlertTriangle />}
        />
        <MetricCard
          ...
          value={
            data.metrics.length
              ? String(data.metrics.reduce((sum, item) => sum + item.recovery_count, 0))
              : '—'
          }
          context="Across selected run"
          tone="accent"
          icon={<RotateCcw />}
        />
      </section>
      ...
      <div className="overview-footer">
        <button type="button" className="insight-link" onClick={() => onNavigate('/clients')}>
          <Users />
          <span>
            <strong>Investigate clients</strong>
            <small>Open per-client evidence and history</small>
          </span>
        </button>
        <button type="button" className="insight-link" onClick={() => onNavigate('/recovery')}>
          <ShieldCheck />
          <span>
            <strong>Review recovery</strong>
            <small>Inspect selective restoration decisions</small>
          </span>
        </button>
      </div>
